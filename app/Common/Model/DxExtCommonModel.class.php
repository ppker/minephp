...
class DxExtCommonModel extends Model {
    ...
    // 		}
        ...
        private function getOneListField($key,$field){
            ...
            if(isset($field["valChange"]["model"])){
                ...
                //dump($field["valChange"]["model"]);exit;
                //得到字典表缓存数据
                //dump($m);die;
                //$m->ee();
    ...
    //$model->getModelInfo("title")
    	...
    	public function fieldToGridField(){
    		$gridFields		= array(); // sigma具体数据
    		if($this->getModelInfo("hasCheckBox")){ //加入选项框 多选
    			$gridFields[0]	= array("id"=>"chk","isCheckColumn"=>true,"header"=>"全选");
    		}
    		$datasetFields	= array(); // sigma数据定义
    		$lFields		= $this->getListFields(); // 经过一轮转化 主要是valChange enum default这类
    		...
    		foreach($lFields as $fieldNameKey	=> $field){
    			if(!($field["hide"] & self::HIDE_FIELD_LIST)){
                    ...
                    $gridField = array ( // sigma 字段
                        "id" => $fieldName,
                        "header" => $gridHeader,
                        "frozen" => ( bool ) ($field ["frozen"]),
                        "grouped" => ( bool ) ($field ["grouped"]),
                        "width" => $field ["width"] ,
                    	'toolTip'=>(bool)($field['toolTip']),
                    );
    	...
    	public function getCacheDictTableData(){
    		...
    		//echo 'ssss';exit;
    		// dump(C("USER_AUTH_KEY"));die;
    		$userId	= intval(session(C("USER_AUTH_KEY"))); // 1
    		//如果没有初始化
    		
    		//dump($userId);exit;
    		// dump($this->cacheDictDatas);exit;
    		
    		if(empty($this->cacheDictDatas)){
    			//缓存的字典表数据
    			//echo '_fields/'.$this->name."_".$userId."_dict";exit;
        ...
        // 屏蔽掉后置操作 date: 2015-8-17 1:19:24
    ...
    /**
    	...
    	//处理 title居中
    	public function gridfieldset(){
    		foreach ($this->listFields as $key => &$value){
    			$value['title'] = "<center>" . $value['title'] . "</center>";
    		}
    	}
