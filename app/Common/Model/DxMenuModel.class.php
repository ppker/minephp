...
class DxMenuModel extends DxExtCommonModel{
	...
	public function getMyMenuList(){
	    return $this->where(array("type"=>array('in',"menu"),$this->getPk()=>array('in',D("Api/Role")->getMenuID())))->order("order_no asc")->select();
	}
	...
	public function getMyController(){
	    $data	= $this->where(array('menu_id'=>array('in',D("Api/Role")->getMenuID())))->order("order_no asc")->select();
	    return $data;
	}
	public function getMyQuickMenu(){
	    $data	= $this->where(array("type"=>"quick_menu",'id'=>array('in',D("Api/Role")->getMenuID())))->order("order_no asc")->select();
	    //die($this->getLastSQL());
	    return $data;
	}
	public function getRoleDeskTop(){
	    $data	= $this->where(array("is_desktop"=>"1",'id'=>array('in',D("Api/Role")->getMenuID())))->order("order_no asc")->select();
	    return $data;
	}
